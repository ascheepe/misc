...
static void hash_table_add(struct hash_table *ht, struct hash_item *item) {
    size_t hash;
...
static struct hash_item *hash_table_get(struct hash_table *ht,
                                        const char *key) {
...
static void hash_table_foreach(struct hash_table *ht,
                               void (*item_function)(struct hash_item *)) {
    size_t i;
...
static void hash_table_grow(struct hash_table **htpp) {
...
#if STATS
static void hash_table_statistics(struct hash_table *ht) {
...
struct hash_item **sort_hash_table(struct hash_table *ht,
    ...
    sorted = xcalloc(ht->item_count, sizeof(*sorted));

    for (i = item_count = 0; i < ht->size; ++i) {
        struct hash_item *item = ht->items[i];
    ...
    qsort(sorted, item_count, sizeof(*sorted), compare_function);

    return sorted;
}

void show_topn(struct hash_table *ht, size_t top_count) {
    struct hash_item **sorted;
    size_t i;

    sorted = sort_hash_table(ht, by_count_descending);

    if (ht->item_count < top_count) {
        top_count = ht->item_count;
    }
...
static void add_word(struct hash_table **htpp, const char *word) {
    struct hash_table *ht = *htpp;
    struct hash_item *item;
...
int main(void) {
    ...
    show_topn(wordcounts, TOP_COUNT);
